...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="position: fixed;">
      ...
      <form  method="POST" action="/PostAdmin" enctype="multipart/form-data">
      {{csrf_field()}}
...
<div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="position: fixed;">
      ...
      <form   method="POST" action="PostAdmin" id="editForm">
      {{csrf_field()}}
      {{method_field('PUT')}}
        <div class="modal-body">
            <div class="row">
    ...
    <div class="container">
        <div class="table-wrapper">
            ...
            <ul class="nav nav-tabs" >
                    <li class="active"><a data-toggle="tab" href="#home">اعلانات المسؤول</a></li>
                    <li><a data-toggle="tab" href="#menu1">اعلانات الشركات </a></li>
                            ...
                            @foreach($Posts as $Post)
                             <tr>
                                ...
                                <a  class="edit "  value="تعديل"><i class="material-icons" style="margin-top: 10px;">&#xE254;</i></a>
                                 ...
                                 <a>
                                    <form class="deleteMessage m-2" method="post" action="/PostAdmin/{{$Post->id}}" >
                                       {{ csrf_field() }}
                                        {{ method_field('delete') }}
                        ...
                        <tbody>
                            ...
                            @foreach($PostsUser as $PostUser)
                             <tr>
                                ...
                                <td>{{$PostUser->title}}</td>
                                <td>{{$PostUser->body}}</td>
                                @if($PostUser->img)
                                    <td> 
                                     <img src="{{ url('storage/'.$PostUser->img) }}" height=200px; alt="{{$PostUser->img_title}}" title="{{ $PostUser->img_title }}"></td>
                                     <td hidden>{{$PostUser->img}}</td>
                                @else  
                                    <td>
                                    </td>
                                @endif
                                <td hidden>{{$PostUser->id}}</td>

                                <td style=" width: 100%;">

                                 <a href="#" class="view" title="View" data-toggle="tooltip" style="margin-top: 10px;"><i class="material-icons">&#xE417;</i></a>
                                 <a >
                                    <form class="deleteMessage m-2" method="post" action="/PostAdmin/{{$PostUser->id}}" >
                                       {{ csrf_field() }}
                                        {{ method_field('delete') }}
                                          <input type="submit" class="btn btn-danger material-icons" value="&#xE872;" >
                                     </form>
    ...
    <script src="/vendor/unisharp/laravel-ckeditor/ckeditor.js"></script>
     ...
     $(document).ready(function() {
         ...
         $('.edit').on('click', function () {
          ...
          $('#editForm').attr('action','/PostAdmin/'+data[4]);
